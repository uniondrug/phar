...
# consul health
curl -s http://172.16.0.67:8086/consul.health | awk -F ',' '{print "8086-customer.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8087/consul.health | awk -F ',' '{print "8087-project.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8088/consul.health | awk -F ',' '{print "8088-product.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8089/consul.health | awk -F ',' '{print "8089-user.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8090/consul.health | awk -F ',' '{print "8090-equity.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8093/consul.health | awk -F ',' '{print "8093-data.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.66:8095/consul.health | awk -F ',' '{print "8095-rule.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8118/consul.health | awk -F ',' '{print "8118-insure.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8119/consul.health | awk -F ',' '{print "8119-wx.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.65:8120/consul.health | awk -F ',' '{print "8120-stagnation.backend{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.64:8210/consul.health | awk -F ',' '{print "8210-notify.backend{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.65:8213/consul.health | awk -F ',' '{print "8213-wx.backend{"$1","$5", "$8", "$9", "$10", "$11", "$12}' && \
curl -s http://172.16.0.67:8215/consul.health | awk -F ',' '{print "8215-stagnation.module{"$1","$5", "$8", "$9", "$10", "$11", "$12}'
